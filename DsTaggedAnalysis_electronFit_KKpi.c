...
double deltaECut_range=0.019; // 0.019
...
int DsTaggedAnalysis_electronFit_KKpi()
{
  TFile *signalFile;
  if (dsCharge>0) signalFile = new TFile("DssteeBIGFILES/MyDChainFile_MC_vtosll_Dsp_KKpi.root");
  else signalFile = new TFile("DssteeBIGFILES/MyDChainFile_MC_vtosll_Dsm_KKpi.root");
  signalFile->cd("DsTaggedDecaysProc");
  TTree *signalTree = (TTree*)gDirectory->Get("nt3");
  
  TFile *converFile;
  if (dsCharge>0) converFile = new TFile("DssteeBIGFILES/MyDChainFile_MCgamma_Dsp_KKpi.root");
  else converFile = new TFile("DssteeBIGFILES/MyDChainFile_MCgamma_Dsm_KKpi.root");
  converFile->cd("DsTaggedDecaysProc");
  TTree *converTree = (TTree*)gDirectory->Get("nt3");
  
  TFile *physicFile = new TFile("DssteeBIGFILES/DsTaggedProc_Data_230474_232255.root");
  ...
  TH1D *h_pi0Mass_signal = new TH1D("h_pi0Mass_signal", "h_pi0Mass_signal", 100, 0.0, 0.4);
          ...
          if (MBCCut(MBC_signal))
          {
            ...
            if (DeltaMCut(DeltaM_signal))
            {
              ...
              if (dD0(d0_e_signal-d0_p_signal))
              {
                ...
                if (fabs(dPhi)<2.) h_dPhi_signal->Fill(dPhi);
                ...
                if (dPhiCut(dPhi))
                {
                  ...
                  if (m_pi0_signal!=-1000) h_pi0Mass_signal->Fill(m_pi0_signal); else large+=1;
                }
              }
            }
          }
          ...
          if (MBCCut(MBC_conver))
          {
            ...
            if (DeltaMCut(DeltaM_conver))
            {
              ...
              if (dD0(d0_e_conver-d0_p_conver))
              {
                ...
                if (fabs(dPhi)<2.) h_dPhi_conver->Fill(dPhi);
          ...
          if (MBCCut(MBC_physic))
          {
            ...
            if (DeltaMCut(DeltaM_physic))
            {
              ...
              if (dD0(d0_e_physic-d0_p_physic))
              {
                ...
                if (fabs(dPhi)<2.) h_dPhi_physic->Fill(dPhi);
                std::cout<<"physic dPhi ="<<dPhi<<std::endl;
                if (dPhiCut(dPhi))
                {
  ...
  line = new TLine(xmax, ymin, xmin, ymin); line->Draw();
  line = new TLine(xmin, ymin, xmin, ymax); line->Draw();
  gPad->Print("dPhi_diffD0_signal_KKpi.png");
  dPhi_diffD0->cd(2);
  ...
  line = new TLine(xmax, ymin, xmin, ymin); line->Draw();
  line = new TLine(xmin, ymin, xmin, ymax); line->Draw();
  gPad->Print("dPhi_diffD0_conver_KKpi.png");
  dPhi_diffD0->cd(3);
  ...
  line = new TLine(xmax, ymin, xmin, ymin); line->Draw();
  line = new TLine(xmin, ymin, xmin, ymax); line->Draw();
  gPad->Print("dPhi_diffD0_physic_KKpi.png");
  ...
  diffZ0->Divide(1,2);
  diffZ0->cd(1);
  h_diffZ0_signal->Draw("SAME");
  diffZ0->cd(2);
  h_diffZ0_conver->Draw("SAME");
  
  TCanvas *pi0Mass = new TCanvas("pi0Mass");
  h_pi0Mass_signal->Draw();
