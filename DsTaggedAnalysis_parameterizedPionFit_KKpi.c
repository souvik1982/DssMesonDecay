...
double deltaECut_range=0.03; // 0.019
double mbcCut_center=2.112, mbcCut_range=0.01;
double deltaMCut_center=0.1435, deltaMCut_range=0.013; // 0.1455, 0.0085
double diffD0Cut=-0.005;
double dPhiCutLess=0.15;
...
int DsTaggedAnalysis_parameterizedPionFit_KKpi()
{

  TFile *signalFile = new TFile("MyDChainFile_MC_vtosll_pionFit_KKpi.root");
  signalFile->cd("DsTaggedDecaysProc");
  TTree *signalTree = (TTree*)gDirectory->Get("nt2");
  
  TFile *converFile = new TFile("DssteeBIGFILES/MyDChainFile_MCgamma_pionFit_KKpi.root");
  converFile->cd("DsTaggedDecaysProc");
  TTree *converTree = (TTree*)gDirectory->Get("nt2");
