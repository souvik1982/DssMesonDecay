...
int DsTaggedAnalysis_electronFit_KsK()
{

  TFile *signalFile;
  if (dsCharge>0) signalFile = new TFile("/nfs/cor/an2/souvik/MC_vtosll_Dsp_KsK/DsTaggedDecaysProc_MC_vtosll_Dsp_KsK.root");
  else signalFile = new TFile("/nfs/cor/an2/souvik/MC_vtosll_Dsm_KsK/DsTaggedDecaysProc_MC_vtosll_Dsm_KsK.root");
  signalFile->cd("DsTaggedDecaysProc");
  TTree *signalTree = (TTree*)gDirectory->Get("nt3");  
  
  TFile *converFile;
  if (dsCharge>0) converFile = new TFile("/nfs/cor/an2/souvik/MC_gamma_Dsp_KsK/DsTaggedDecaysProc_MC_gamma_Dsp_KsK.root");
  else converFile = new TFile("/nfs/cor/an2/souvik/MC_gamma_Dsm_KsK/DsTaggedDecaysProc_MC_gamma_Dsm_KsK.root");
  converFile->cd("DsTaggedDecaysProc");
  TTree *converTree = (TTree*)gDirectory->Get("nt3");
  
  TFile *physicFile = new TFile("/nfs/cor/an2/souvik/Dataset47/DsTaggedProc_Data_230474_232255.root");
  physicFile->cd("DsTaggedDecaysProc");
  TTree *physicTree = (TTree*)gDirectory->Get("nt3");
  ...
  std::cout<<"Number of signal events after KsK tag = "<<signalEvents.tagCut<<std::endl;
  ...
  std::cout<<"Number of conver events after KsK tag = "<<converEvents.tagCut<<std::endl;
  ...
  std::cout<<"Number of physic events after KsK tag = "<<physicEvents.tagCut<<std::endl;
  ...
  line = new TLine(xmax, ymin, xmin, ymin); line->Draw();
  line = new TLine(xmin, ymin, xmin, ymax); line->Draw();
  gPad->Print("dPhi_diffD0_signal_KsK.png");
  dPhi_diffD0->cd(2);
  ...
  line = new TLine(xmax, ymin, xmin, ymin); line->Draw();
  line = new TLine(xmin, ymin, xmin, ymax); line->Draw();
  gPad->Print("dPhi_diffD0_conver_KsK.png");
  dPhi_diffD0->cd(3);
  ...
  line = new TLine(xmax, ymin, xmin, ymin); line->Draw();
  line = new TLine(xmin, ymin, xmin, ymax); line->Draw();
  gPad->Print("dPhi_diffD0_physic_KsK.png");
