...
double deltaECut_center=0.012, deltaECut_range=0.02; // 0.03
double mbcCut_center=2.112, mbcCut_range=0.008;
double deltaMCut_center=0.158, deltaMCut_range=0.009;
...
int DsTaggedAnalysis_GenericContinuumPhysics_KKpi()
{
  ...
  std::cout<<"Number of generic events after KKpi tag = "<<genericEvents.tagCut<<std::endl;
  ...
  std::cout<<"Number of continu events after KKpi tag = "<<continuEvents.tagCut<<std::endl;
  ...
  std::cout<<"Number of physics events after KKpi tag = "<<physicsEvents.tagCut<<std::endl;
  ...
  line = new TLine(xmin,ymax,xmin,0); line->Draw();
  line = new TLine(xmax,ymax,xmax,0); line->Draw();
  gPad->Print("mBC_generic_KKpi.png");
  MBC->cd(2);
  h_MBC_continu->Draw();
  ymax=(h_MBC_continu->GetMaximum())*0.75;
  line = new TLine(xmin,ymax,xmin,0); line->Draw();
  line = new TLine(xmax,ymax,xmax,0); line->Draw();
  gPad->Print("mBC_continu_KKpi.png");
  MBC->cd(3);
  h_MBC_physics->Draw();
  ymax=(h_MBC_physics->GetMaximum())*0.75;
  line = new TLine(xmin,ymax,xmin,0); line->Draw();
  line = new TLine(xmax,ymax,xmax,0); line->Draw();
  gPad->Print("mBC_physics_KKpi.png");
  ...
  line = new TLine(xmin,ymax,xmin,0); line->Draw();
  line = new TLine(xmax,ymax,xmax,0); line->Draw();
  gPad->Print("DeltaM_generic_KKpi.png");
  DeltaM->cd(2);
  h_DeltaM_continu->Draw();
  ymax=(h_DeltaM_continu->GetMaximum())*0.75;
  line = new TLine(xmin,ymax,xmin,0); line->Draw();
  line = new TLine(xmax,ymax,xmax,0); line->Draw();
  gPad->Print("DeltaM_continu_KKpi.png");
  DeltaM->cd(3);
  h_DeltaM_physics->Draw();
  ymax=(h_DeltaM_physics->GetMaximum())*0.75;
  line = new TLine(xmin,ymax,xmin,0); line->Draw();
  line = new TLine(xmax,ymax,xmax,0); line->Draw();
  gPad->Print("DeltaM_physics_KKpi.png");
  
  TCanvas *d0 = new TCanvas("d0");
  d0->Divide(1,4);
  d0->cd(1);
  //h_d0_e_generic->Scale(1/h_d0_e_generic->GetEntries());
  //h_d0_e_continu->Scale(1/h_d0_e_continu->GetEntries());
  h_d0_e_generic->Draw();
  d0->cd(2);
  h_d0_e_continu->Draw();
  d0->cd(3);
  //h_d0_p_generic->Scale(1/h_d0_p_generic->GetEntries());
  //h_d0_p_continu->Scale(1/h_d0_p_continu->GetEntries());
  h_d0_p_generic->Draw();
  d0->cd(4);
  h_d0_p_continu->Draw();
  ...
  gPad->Print("dPhi_generic_KKpi.png");
  dPhi->cd(2);  
  h_dPhi_continu->Draw();
  ymax=(h_dPhi_continu->GetMaximum())*0.75;
  line = new TLine(dPhiCutLess,ymax,dPhiCutLess,0); line->Draw();
  gPad->Print("dPhi_continu_KKpi.png");
  dPhi->cd(3);
  h_dPhi_physics->Draw();
  ymax=(h_dPhi_physics->GetMaximum())*0.75;
  line = new TLine(dPhiCutLess,ymax,dPhiCutLess,0); line->Draw();
  gPad->Print("dPhi_physics_KKpi.png");
  ...
  if (DsMass_sideband)
  {
    ...
    std::cout<<" after signal/5 = "<<continu_afterSignal/5<<std::endl;
    std::cout<<"Data: ";
    double physics_beforeSignal=h_dsPlusM_physics->Integral(h_dsPlusM_physics->GetXaxis()->GetFirst(), h_dsPlusM_physics->GetXaxis()->FindBin(dsPlusMCut_center-dsPlusMCut_range));
    double physics_afterSignal=h_dsPlusM_physics->Integral(h_dsPlusM_physics->GetXaxis()->FindBin(dsPlusMCut_center+dsPlusMCut_range), h_dsPlusM_physics->GetXaxis()->GetLast());
    std::cout<<" before signal = "<<physics_beforeSignal;
    std::cout<<" after signal = "<<physics_afterSignal<<std::endl;
  }
